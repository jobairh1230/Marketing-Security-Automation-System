...
# Add the root directory to sys.path so that 'app' module can be found
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# Import necessary functions from the app modules
from app.marketing.generate import generate_marketing_content
from app.marketing.store import create_draft, list_items, update_status, get_item, init_db
...
with col2:
    if "generated_content" in st.session_state:
        ...
        if st.button("Save as Draft"):
            # Save the content as draft in the database
...
# Retrieve list of draft items from the database
items = list_items()

if items:
    df = pd.DataFrame([{
        "id": i[0],               # id from the DB
        "status": i[3],           # status (DRAFT, APPROVED, PUBLISHED)
        "type": i[2],             # content_type
        "prompt": i[4],           # prompt file
        "brief": (i[5][:60] + "...") if len(i[5]) > 60 else i[5],  # brief text, truncated
    } for i in items])
    ...
    with colA:
        if st.button("View Item"):
            ...
            if item:
                st.write(f"**Status:** {item[3]}")
                st.code(item[5], language="json")  # Display the JSON content
            else:
...
# import os
# sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))
# from app.marketing.store import init_db
